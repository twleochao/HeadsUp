import json
from pydantic import BaseModel, Field
from typing import Dict, Tuple, Optional

Rect = Tuple[int, int, int, int]

class SeatROIs(BaseModel):
    seat_anchor: Rect = Field(..., description="Main anchor for the seat, used for relative calcs")
    bet: Rect = Field(..., description="Area where the bet amount is displayed")
    stack: Rect = Field(..., description="Area where the stack size is displayed")
    cards: Rect = Field(..., description="Area covering the player's hole cards")
    name: Rect = Field(..., description="Player name area")
    is_hero: bool = False

class BoardROIs(BaseModel):
    flop_1: Rect
    flop_2: Rect
    flop_3: Rect
    turn: Rect
    river: Rect
    pot: Rect
    
class DealerButtonROI(BaseModel):
    position_anchor: Rect

class RoiProfile(BaseModel):
    profile_name: str
    resolution: Tuple[int, int] = Field(..., description="Table window resolution (w, h)")
    board: BoardROIs
    dealer_button: DealerButtonROI
    seats: Dict[int, SeatROIs]

    def save_json(self, filepath: str) -> None:
        try:
            with open(filepath, 'w') as f:
                json.dump(self.model_dump(), f, indent=2)
        except IOError as e:
            print(f"Error saving ROI profile to {filepath}: {e}")
            raise

    @classmethod
    def load_json(cls, filepath: str) -> 'RoiProfile':
        try:
            with open(filepath, 'r') as f:
                data = json.load(f)
            return cls(**data)
        except FileNotFoundError:
            print(f"Error: ROI profile not found at {filepath}")
            raise
        except json.JSONDecodeError:
            print(f"Error: Could not decode JSON from {filepath}")
            raise
        except Exception as e:
            print(f"An unexpected error occurred loading {filepath}: {e}")
            raise

    def get_seat_roi(self, seat_index: int) -> Optional[SeatROIs]:
        return self.seats.get(seat_index)